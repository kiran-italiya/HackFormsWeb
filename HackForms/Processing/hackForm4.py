...
def hackForm(csvfile, height):
    ...
    df['visited'] = 0
    while df[df['visited'] == 0].shape[0] > 0:  # element < df.shape[0]-1:
        element += 1;labels = 0;fields = 0;checkboxes = 0;radios = 0;in_strip_elements = 0
        ...
        curr_row = df.loc[visIndex]

        curr_df = MyUtils.MyUtils.allInline(curr_row[1], curr_row[1] + curr_row[2], df)
        curr_df = curr_df.sort_values(by='left')  # .reset_index(drop=True)

        curr_df = curr_df[curr_df['visited'] == 0]
        ...
        # labels are present in the strip
            ...
            if fields > 0:
                if checkboxes == 0 and radios == 0:
                    if curr_df.iloc[0].type == "label":
                        LFLFFF(df,
                               curr_df)  # parent_group if is_valid_parent_group(parent_group,topy,bottomy) else None)
                    else:
                        if labels == fields:
                            FLFLFL(df, curr_df)
                        else:
...
def is_valid_parent_group(df, strip_top, strip_bottom, parent_group):
    if parent_group is not None:
        ...
        if df.loc[parent_group].top > (2.2) * strip_top - strip_bottom and df.loc[parent_group].top + df.loc[
            parent_group].height < strip_top:
            return True
        else:
            # parent_group = None
            return False
