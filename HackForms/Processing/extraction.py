                ...
                if group != curr_group[0]:
                    ...
                    try:
                        ...
                        if df_temp.iloc[0]['no'] < 0:
                            ...
                            df_final.at[length, df.loc[df_temp.iloc[0]['no']][
                                'value']] = 1  # df.loc[df_temp.iloc[0]['no']]['value']
                            try:
            ...
            try:
                if group != curr_group[0]:
                    ...
                    df_temp = df_temp.append({'sum': np.sum(arr), 'no': abs(curr_group[1]), 'group': curr_group[0]},
                                             ignore_index=True)
                    cv2.imshow("crop2", threshold[y + int(0.25 * h):y + h - int(0.25 * h),
                    ...
                    df_temp = df_temp.append({'sum': np.sum(arr), 'no': curr_group[1], 'group': abs(curr_group[0])},
                                             ignore_index=True)
                    cv2.imshow("crop3", threshold[y + int(0.25 * h):y + h - int(0.25 * h),
                                        x + int(0.25 * w):x + w - int(0.25 * w)])
                    cv2.waitKey(0)
                    cv2.destroyAllWindows()
            except Exception as e:
...
def perform_OCR(img, df, df_final, length):
    ...
    for i, row in fieldsDf.iterrows():
        ...
        # cv2.waitKey(0)
        result = pytesseract.image_to_string(image=cropped_img, lang='eng', config='--psm 6')
